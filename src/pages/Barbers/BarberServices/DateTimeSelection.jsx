...
const DateTimeSelection = ({ selectedDate, selectedTime, handleDateSelect, handleTimeSelect, workingHours }) => {
  ...
  return (
    <div className="mt-8">
      <h2 className="text-2xl font-bold mb-6 px-2 text-slate-900 dark:text-gray-100">Select Date & Time</h2>
      <div className="space-y-6">
        <div className="space-y-4">
          <h3 className="font-semibold flex items-center px-4 text-slate-900 dark:text-gray-100">
          ...
          <div className="flex justify-center">
            <Calendar
              ...
              className="border border-gray-200 dark:border-slate-700 rounded-lg shadow-sm dark:bg-slate-800 dark:text-gray-100"
            />
          </div>
        </div>
        ...
        {selectedDate && (
          <div className="space-y-4 px-4">
            <h3 className="font-semibold text-slate-900 dark:text-gray-100">Select Time</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {generateTimeSlots().map((time) => (
                <button
                  key={time}
                  onClick={() => handleTimeSelect(time)}
                  className={`p-3 rounded text-sm font-medium transition-colors ${selectedTime === time
                    ? 'bg-blue-500 dark:bg-blue-600 text-white'
                    : 'bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-slate-900 dark:text-gray-100'
                    }`}
                >
                  {time}
                </button>
