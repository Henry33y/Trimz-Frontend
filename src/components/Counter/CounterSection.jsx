...
const Counter = ({ end, duration, title, colorClass }) => {
  ...
  return (
    <div>
      <h2 className="text-[36px] leading-[56px] lg:text-[44px] lg:leading-[54px] font-[700] text-headingColor dark:text-gray-100">
        {count}{end.toString().includes('+') ? '+' : '%'}
      </h2>
      <span className={`w-[100px] h-2 ${colorClass} rounded-full block mt-[-14px]`}></span>
      <p className="text__para dark:text-gray-300">{title}</p>
    </div>
  );
};
